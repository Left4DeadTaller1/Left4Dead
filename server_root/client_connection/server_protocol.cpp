...
/*std::vector<int> ServerProtocol::receive_end_move(bool& was_closed, Socket &peer){

}*/

/*void ServerProtocol::send_int8(int8_t number, bool& was_closed, Socket &peer){
    int8_t buf[1];
    buf[0] = number;
    peer.sendall(buf , 1, &was_closed);
}

void ServerProtocol::send_int32(int32_t number, bool& was_closed, Socket &peer){
    int32_t buf[1];
    buf[0] = number;
...
void ServerProtocol::send_update_game(std::shared_ptr<gameStateDTO_t> gameStateDTO, 
                                        bool& was_closed, 
                                        Socket &peer){
